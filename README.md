...
**起唱提示音（防开口跑调）**：每次练习开始时先打两拍节拍（滴—嘀—），最后一拍用琴音示范首音标准音，提示音正好结束在首音开始的时刻，跟着示范开口就不容易起调。提示音期间的麦克风帧会被 `ignoreBefore` 整段丢弃，不会混进音准/节奏统计；设置页可关闭。曲库含《小星星》《欢乐颂》《两只老虎》《生日快乐》及音阶、气息、音域等共 10 个练习。
...
js/
  ...
  lessons.js            10 个练习（含 4 首歌）+ 5 篇课程 + 咬字句子，节拍 → 时间轴
